---
name: spec-update
description: Use when a repository already has .agents/spec files and those existing spec files need a conservative update plan from repository evidence and committed git changes before any approved spec update is applied.
---

# Plan Existing Project Spec Updates

Inspect existing files under `.agents/spec`, gather evidence, and produce a conservative update plan for user review before any spec file is changed.

## Core Rule

No approved plan, no update. `spec-update` must check whether `.agents/spec` exists first, read the existing spec files, gather qualifying committed git changes, and stop after emitting an update plan. It must not modify spec files during the planning command.

## Behavior Contract

- The first step is checking whether `.agents/spec` exists.
- If `.agents/spec` does not exist, `spec-update` must stop and report that no update plan can be generated from this workflow.
- `spec-update` reads the existing spec files under `.agents/spec/*` before proposing changes.
...
- `.agents/spec` already exists and you want to determine whether existing spec files need refresh
...
- The repository does not have `.agents/spec` yet
...
1. Check whether `.agents/spec` exists.
2. If `.agents/spec` exists, recursively read the existing spec files under `.agents/spec/*`.
...
- The planning summary should explicitly say whether `.agents/spec` was found.
...
- The planning command must not modify `.agents/spec`.
...
- Skipping the `.agents/spec` existence check
...
- Creating `.agents/spec` from `spec-update`
